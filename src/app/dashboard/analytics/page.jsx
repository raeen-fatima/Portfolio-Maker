...
//   const { analytics, loading, error, loadAnalytics } = useAnalytics();
...
//     if (topDevice)
//       items.push(
//         `${topDevice.name} is your most common device at ${topDevice.percent}%.`,
//       );
//     if (topSource)
//       items.push(`${topSource.name} sends the most traffic to your portfolio.`);
//     if (!summary.resumeDownloads)
//       items.push(
//         "No resume downloads yet; make the resume action prominent in your hero section.",
//       );
...
//             Live visitor, engagement, device, and traffic data from your
//             published portfolio.
//           </p>
//         </div>
...
//           detail={`${summary.todayViews} today`}
//         />
//         <Metric
//           icon={<Users size={20} />}
//           label="Unique visitors"
//           value={summary.uniqueVisitors}
//           detail={`${summary.monthlyViews} views this month`}
//         />
//         <Metric
//           icon={<Download size={20} />}
//           label="Resume downloads"
//           value={summary.resumeDownloads}
//           detail={`${summary.contactClicks} contact clicks`}
//         />
//         <Metric
...
//                     style={{
//                       height: `${Math.max(item.views ? 8 : 2, (item.views / maxViews) * 100)}%`,
//                     }}
//                   />
//                 </div>
...
//               <div
//                 key={insight}
//                 className="rounded-xl border border-white/10 bg-black/10 p-4 text-sm leading-6 text-zinc-300"
//               >
//                 {insight}
//               </div>
//             ))}
...
export default function AnalyticsPage() {
  ...
  useGSAP(
    () => {
      if (!loading && containerRef.current) {
        // Entrance sequence for headers, metrics, and cards
        gsap.from(".analytics-item", {
          y: 20,
          opacity: 0,
          duration: 0.5,
          stagger: 0.08,
        ...
        // Dynamic bar chart height reveal animation
        ...
        // Progress line widths animation
        gsap.from(".analytics-progress-line", {
          width: "0%",
          duration: 0.8,
          stagger: 0.06,
          ease: "power2.out",
  ...
  return (
    <div ref={containerRef} className="space-y-8">
      <div className="analytics-item flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm uppercase tracking-[0.2em] text-zinc-500">
            Analytics
          </p>
      ...
      {error && (
        <div className="analytics-item rounded-xl border border-red-500/20 bg-red-500/10 p-4 text-red-300">
          {error}
        </div>
      )}
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(280px,1fr)]">
        <section className="analytics-item rounded-2xl border border-white/10 bg-white/[0.03] p-5 sm:p-7">
          <div className="flex items-center justify-between gap-4">
            <div>
              <h2 className="text-xl font-semibold text-white">
                Views overview
          ...
          <div className="mt-8 grid h-64 grid-cols-7 items-end gap-2 sm:gap-4">
            {(analytics?.views || []).map((item) => (
              ...
              >
                ...
                <div className="flex h-48 w-full items-end rounded-lg bg-white/[0.04] p-1">
                  <div
                    className="analytics-bar w-full rounded-md bg-white transition-all"
                    style={{
                      height: `${Math.max(item.views ? 8 : 2, (item.views / maxViews) * 100)}%`,
                    }}
                  />
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <section className="analytics-item rounded-2xl border border-white/10 bg-white/[0.03] p-6">
          <h2 className="text-xl font-semibold text-white">Engagement</h2>
          <div className="mt-6 space-y-4">
            <Engagement label="Contact clicks" value={summary.contactClicks} />
            <Engagement label="GitHub clicks" value={summary.githubClicks} />
...
function Metric({ icon, label, value, detail }) {
  return (
    <div className="analytics-item rounded-2xl border border-white/10 bg-white/[0.03] p-5">
      <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/[0.06] text-white">
        {icon}
      </div>
      <p className="mt-5 text-3xl font-bold text-white">
        ...
        {items.length ? (
          items.slice(0, 5).map((item) => (
            <div key={item.name}>
              ...
              <div className="h-2 overflow-hidden rounded-full bg-white/[0.06]">
                <div
                  className="analytics-progress-line h-full rounded-full bg-white"
                  style={{ width: `${item.percent}%` }}
                />
              </div>
            </div>
